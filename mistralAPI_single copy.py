...
if __name__ == "__main__":
    ...
    for key in model.chi_json.keys():
        ...
        print(key)
        if key != "CHI_SUMMARY":
            new_result[key] = model.execute(image_path=image_file,sys_prompt=model.chi_json[key]["system_prompt"],prompt=model.chi_json[key]["user_prompt"])
    result = new_result
    # serialize result to string
    result_str = json.dumps(result)
    summary_result = model.chi_json["CHI_SUMMARY"]["user_prompt"] + result_str
    result["CHI_SUMMARY"] = model.execute(image_path=image_file,sys_prompt=model.chi_json["CHI_SUMMARY"]["system_prompt"],prompt=summary_result)
